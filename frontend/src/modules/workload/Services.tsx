...
import { ShareAltOutlined, ReloadOutlined, DeleteOutlined, EditOutlined } from '@ant-design/icons'
  ...
  return (
    <div>
      ...
      <EditResourceModal open={!!editTarget} onClose={() => setEditTarget(null)} onSuccess={() => { refetch(); setEditTarget(null) }}
        kind="Service" apiVersion="v1" namespace={editTarget?.namespace} name={editTarget?.name || ''} title={`编辑 Service: ${editTarget?.name}`} />
    </div>
  )
}
